...
// ─── Pantalla principal ───────────────────────────────────────────────────────
const OptionsScreenAxis = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenAxis'>>();
  const params = route.params ?? { category: 'nodeType' };
  ...
  // Movidos al interior del componente para poder usar t().
  // Las opciones estáticas como fallback para categorías conocidas.
  // Para nodeIds las opciones llegan dinámicamente via route.params.options.
  const staticOptionsData = useMemo<Record<string, string[]>>(
    () => ({
      nodeType: [t('axis.nodeType.nodo'), t('axis.nodeType.tanque'), t('axis.nodeType.reservorio')],
    }),
    [t]
  );
  ...
  const titleMap = useMemo<Record<string, string>>(
    () => ({
      ...
      modoNodo:    t('axis.options.title.modoNodo'),
      modoTuberia: t('axis.options.title.modoTuberia'),
    }),
    [t]
  );
  ...
  const subtitleMap = useMemo<Record<string, string>>(
    () => ({
      ...
      modoNodo:    t('axis.options.selectOption'),
      modoTuberia: t('axis.options.selectOption'),
    }),
    [t]
  );

  const category         = params.category ?? 'nodeType';
  ...
  // Las opciones vienen de los params (dinámicas) o del mapa estático como fallback.
  const options = useMemo(
    () => params.options ?? staticOptionsData[category] ?? [],
    [params.options, category, staticOptionsData]
  );

  const title    = useMemo(() => titleMap[category]    ?? category, [category, titleMap]);
  const subtitle = useMemo(() => subtitleMap[category] ?? t('axis.options.selectOption'), [category, subtitleMap, t]);
  ...
  // Para AxisScreen las opciones ya son etiquetas legibles (p.ej. 'Nodo', 'N1'),
  // así que displayLabel es igual al valor.
...
export default OptionsScreenAxis;