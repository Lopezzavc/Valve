...
const OPTIONS_DATA: Record<string, string[]> = {
  ...
  flowRate: ['m³/s', 'L/s', 'L/min', 'gal/min', 'ft³/s', 'cm³/s'],
};

// Claves de título/subtítulo para Bernoulli
const TITLE_I18N_KEY: Record<string, string> = {
  density: 'bernoulliCalc.labels.density',
  pressure: 'optionsScreen.titles.pressure',
  length: 'optionsScreen.titles.length',
  velocity: 'continuidadCalc.labels.velocity',
  flowRate: 'bernoulliCalc.labels.flowRate',
};
const SUBTITLE_I18N_KEY: Record<string, string> = {
  ...
  flowRate: 'optionsScreen.subtitles.units',
};
...
const OptionsScreenBernoulli = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenBernoulli'>>();
  const params = route.params ?? { category: 'density' };
  ...
  const category = params.category ?? 'density';
  ...
  const getItemLayout = useCallback(
    (_: any, index: number) => ({
      length: 46, // ~ item minHeight(45) + separator(1)
      offset: 47 * index,
      index,
    }),
    []
...
const styles = StyleSheet.create({
  ...
  export default OptionsScreenBernoulli;