...
// ─── Options per category (matching PotenciaCalc conversionFactors) ────────────
const OPTIONS_DATA: Record<string, string[]> = {
  length:        ['m', 'mm', 'cm', 'km', 'in', 'ft', 'yd'],
  flow:          ['m³/s', 'L/s', 'm³/h', 'ft³/s', 'gal/min'],
  viscosity:     ['m²/s', 'cm²/s', 'mm²/s', 'ft²/s', 'cSt'],
  specificWeight:['N/m³', 'kN/m³', 'lbf/ft³'],
  pressure:      ['Pa', 'kPa', 'MPa', 'bar', 'atm', 'psi', 'mmHg', 'mca'],
};

// ─── i18n keys — category title shown at the top of the picker ────────────────
const TITLE_I18N_KEY: Record<string, string> = {
  ...
  specificWeight: 'optionsScreen.titles.specificWeight',
...
const SUBTITLE_I18N_KEY: Record<string, string> = {
  ...
  specificWeight: 'optionsScreen.subtitles.units',
...
// ─── Main screen ───────────────────────────────────────────────────────────────
const OptionsScreenPotencia = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenPotencia'>>();
  ...
  const getDisplayLabel = useCallback((_cat: string, value: string): string => value, []);
...
export default OptionsScreenPotencia;