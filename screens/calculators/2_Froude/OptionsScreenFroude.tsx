...
const OPTIONS_DATA: Record<string, string[]> = {
  area: ['m²', 'cm²', 'mm²', 'km²', 'ft²', 'in²', 'yd²', 'mi²'],
  length: ['m', 'mm', 'cm', 'km', 'in', 'ft', 'yd', 'mi'],
  velocity: ['m/s', 'km/h', 'ft/s', 'mph', 'kn', 'cm/s', 'in/s'],
  acceleration: ['m/s²', 'ft/s²', 'km/h²', 'cm/s²'],
};

const TITLE_I18N_KEY: Record<string, string> = {
  area: 'froudeCalc.labels.area',
  length: 'froudeCalc.labels.width',
  velocity: 'froudeCalc.labels.velocity',
  acceleration: 'froudeCalc.labels.gravity',
};
...
const OptionsScreenFroude = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenFroude'>>();
  const params = route.params ?? { category: 'area' };
  ...
  const category = params.category ?? 'area';
...
export default OptionsScreenFroude;