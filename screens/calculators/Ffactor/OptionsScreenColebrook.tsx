...
const OPTIONS_DATA: Record<string, string[]> = {
  ...
  dynamicViscosity: ['Pa·s', 'cP', 'P', 'kg/(m·s)', 'lb/(ft·s)'],
  pipeMaterial: ['Personalizado', 'Acero comercial', 'Hierro fundido', 'Concreto', 'PVC', 'Cobre', 'Acero galvanizado'],
};

// Claves de título/subtítulo para Colebrook
const TITLE_I18N_KEY: Record<string, string> = {
  density: 'colebrookCalc.labels.density',
  length: 'optionsScreen.titles.length',
  velocity: 'colebrookCalc.labels.velocity',
  dynamicViscosity: 'colebrookCalc.labels.dynamicViscosity',
  pipeMaterial: 'colebrookCalc.labels.material',
};
const SUBTITLE_I18N_KEY: Record<string, string> = {
  ...
  pipeMaterial: 'optionsScreen.subtitles.selection',
};
...
const OptionsScreenColebrook = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenColebrook'>>();
  ...
  const getDisplayLabel = useCallback((cat: string, value: string): string => {
    // Para pipeMaterial, traducir los materiales
    if (cat === 'pipeMaterial') {
      switch (value) {
        case 'Personalizado':
          return t('colebrookCalc.materials.custom') || 'Personalizado';
        case 'Acero comercial':
          return t('colebrookCalc.materials.commercialSteel') || 'Acero comercial';
        case 'Hierro fundido':
          return t('colebrookCalc.materials.castIron') || 'Hierro fundido';
        case 'Concreto':
          return t('colebrookCalc.materials.concrete') || 'Concreto';
        case 'PVC':
          return t('colebrookCalc.materials.pvc') || 'PVC';
        case 'Cobre':
          return t('colebrookCalc.materials.copper') || 'Cobre';
        case 'Acero galvanizado':
          return t('colebrookCalc.materials.galvanizedSteel') || 'Acero galvanizado';
        default:
          return value;
      }
    }
    
    // Para las demás categorías (unidades), mostrar tal como están
    return value;
  }, [t]);
...
// Styles (idénticos a OptionsScreenBernoulli)
...
export default OptionsScreenColebrook;