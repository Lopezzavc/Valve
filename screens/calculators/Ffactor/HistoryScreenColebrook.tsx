...
const buildInputsString = (parsedInputs: ParsedInputs, t: (k: string, vars?: any) => string) => {
  ...
  // Modo de cálculo
  inputString += `${t('colebrookCalc.mode.label') || 'Modo'}: ${mode === 'Simple' ? (t('colebrookCalc.mode.simple') || 'Simple') : (t('colebrookCalc.mode.advanced') || 'Avanzado')}\n`;

  // Características de la tubería
  inputString += `${t('colebrookCalc.pipeCharacteristics') || 'Características de la Tubería'}:\n`;
  inputString += `  ${t('colebrookCalc.labels.material') || 'Material'}: ${material}\n`;
  
  if (roughness && roughness !== 'N/A') {
    inputString += `  ${t('colebrookCalc.labels.roughness') || 'Rugosidad Absoluta'}: ${roughness} ${roughnessUnit}\n`;
  }
  if (diameter && diameter !== 'N/A') {
    inputString += `  ${t('colebrookCalc.labels.diameter') || 'Diámetro Interno'}: ${diameter} ${diameterUnit}\n`;
  }
  if (relativeRoughness && relativeRoughness !== 'N/A') {
    inputString += `  ${t('colebrookCalc.labels.relativeRoughness') || 'Rugosidad Relativa'}: ${relativeRoughness}\n`;
  }
  ...
  if (mode === 'Simple') {
    if (reynolds && reynolds !== 'N/A') {
      inputString += `  ${t('colebrookCalc.labels.reynolds') || 'Número de Reynolds'}: ${reynolds}\n`;
    }
  } else {
    if (density && density !== 'N/A') {
      inputString += `  ${t('colebrookCalc.labels.density') || 'Densidad'}: ${density} ${densityUnit}\n`;
    }
    if (velocity && velocity !== 'N/A') {
      inputString += `  ${t('colebrookCalc.labels.velocity') || 'Velocidad'}: ${velocity} ${velocityUnit}\n`;
    }
    if (dynamicViscosity && dynamicViscosity !== 'N/A') {
      inputString += `  ${t('colebrookCalc.labels.dynamicViscosity') || 'Viscosidad Dinámica'}: ${dynamicViscosity} ${dynamicViscosityUnit}\n`;
    }
    if (reynolds && reynolds !== 'N/A') {
      inputString += `  ${t('colebrookCalc.labels.reynolds') || 'Número de Reynolds'}: ${reynolds}\n`;
    }
...
const buildCopyText = (parsedInputs: ParsedInputs, formattedResult: string, t: (k: string, vars?: any) => string) => {
  ...
  // Factor de fricción
  textToCopy += `${t('colebrookCalc.frictionFactor') || 'Factor de Fricción'}: ${formattedResult}\n\n`;
  ...
  // Modo de cálculo
  textToCopy += `${t('colebrookCalc.mode.label') || 'Modo'}: ${mode === 'Simple' ? (t('colebrookCalc.mode.simple') || 'Simple') : (t('colebrookCalc.mode.advanced') || 'Avanzado')}\n\n`;

  // Características de la tubería
  textToCopy += `${t('colebrookCalc.pipeCharacteristics') || 'Características de la Tubería'}:\n`;
  textToCopy += `  ${t('colebrookCalc.labels.material') || 'Material'}: ${material}\n`;
  
  if (roughness && roughness !== 'N/A') {
    textToCopy += `  ${t('colebrookCalc.labels.roughness') || 'Rugosidad Absoluta'}: ${roughness} ${roughnessUnit}\n`;
  }
  if (diameter && diameter !== 'N/A') {
    textToCopy += `  ${t('colebrookCalc.labels.diameter') || 'Diámetro Interno'}: ${diameter} ${diameterUnit}\n`;
  }
  if (relativeRoughness && relativeRoughness !== 'N/A') {
    textToCopy += `  ${t('colebrookCalc.labels.relativeRoughness') || 'Rugosidad Relativa'}: ${relativeRoughness}\n`;
  }
  ...
  if (mode === 'Simple') {
    if (reynolds && reynolds !== 'N/A') {
      textToCopy += `  ${t('colebrookCalc.labels.reynolds') || 'Número de Reynolds'}: ${reynolds}\n`;
    }
  } else {
    if (density && density !== 'N/A') {
      textToCopy += `  ${t('colebrookCalc.labels.density') || 'Densidad'}: ${density} ${densityUnit}\n`;
    }
    if (velocity && velocity !== 'N/A') {
      textToCopy += `  ${t('colebrookCalc.labels.velocity') || 'Velocidad'}: ${velocity} ${velocityUnit}\n`;
    }
    if (dynamicViscosity && dynamicViscosity !== 'N/A') {
      textToCopy += `  ${t('colebrookCalc.labels.dynamicViscosity') || 'Viscosidad Dinámica'}: ${dynamicViscosity} ${dynamicViscosityUnit}\n`;
    }
    if (reynolds && reynolds !== 'N/A') {
      textToCopy += `  ${t('colebrookCalc.labels.reynolds') || 'Número de Reynolds'}: ${reynolds}\n`;
    }
...
const HistoryCard = React.memo(({ item, isFirst, onDelete }: HistoryCardProps) => {
  ...
  const handleCopy = useCallback(() => {
    const frictionFactor = parseFloat(String(item.result ?? '').replace(',', '.'));
    if (!isFinite(frictionFactor) || frictionFactor === 0) {
      Toast.show({
        type: 'error',
        text1: t('common.error'),
        text2: t('colebrookCalc.toasts.noResultsToCopy') || 'No hay resultados para copiar',
      });
      return;
    }
    ...
    Toast.show({
      ...
      text2: t('colebrookCalc.toasts.copied') || 'Resultados copiados al portapapeles',
    });
        ...
        <View style={[styles.optionsContainer, { backgroundColor: themeColors.card }]}>
          <View style={styles.itemContent}>
            <Text style={[styles.resultLabel, { color: themeColors.text, fontSize: 16 * fontSizeFactor }]}>
              {t('colebrookCalc.frictionFactor') || 'Factor de Fricción'}:
            </Text>
...
const HistoryScreenColebrook = () => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      ...
      // Filtrar solo los cálculos de Colebrook
      const colebrookHistory = fetched.filter((item: HistoryItem) => item.calculation_type === 'colebrook');
      setHistory((prev) => {
        const sameLength = prev.length === colebrookHistory.length;
        if (sameLength) {
          const same = prev.every((p, i) => {
            const n = colebrookHistory[i];
            return (
              p.id === n.id &&
              p.result === n.result &&
              p.inputs === n.inputs &&
      ...
      [
        { text: t('history.deleteAll.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            const db = dbRef.current;
            if (db) {
              // Eliminar solo los elementos de Colebrook del historial actual
          ...
          <>
            ...
            <View style={styles.titlesContainer}>
              <Text style={[styles.subtitle, { color: themeColors.text, fontSize: 18 * fontSizeFactor }]}>
                {t('colebrookCalc.title') || 'Ecuación de Colebrook'}
              </Text>
...
export default HistoryScreenColebrook;