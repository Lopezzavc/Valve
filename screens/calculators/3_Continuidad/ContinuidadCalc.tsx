...
const ContinuidadCalc: React.FC = () => {
  ...
  const getMainResultLabel = useCallback(() => {
    if (state.unknownVariable) {
      return `${state.unknownVariable.name} (${state.unknownVariable.unit})`;
    }
    return `Q (m³/s)`;
  }, [state.unknownVariable]);
  ...
  const getActiveValue = useCallback((): string => {
    ...
    const map: Record<string, string> = {
      ...
      fillHeight: s.fillHeight,
      velocityCaudal: s.velocityCaudal,
      A1: s.A1,
      V1: s.v1,
      A2: s.A2,
      V2: s.v2,
    ...
    <View style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.titlesContainer}>
          <Text style={[styles.subtitle, { fontSize: 18 * fontSizeFactor }]}>{t('continuidadCalc.calculator') || 'Calculadora'}</Text>
          <Text style={[styles.title, { fontSize: 30 * fontSizeFactor }]}>{t('continuidadCalc.title') || 'Continuidad'}</Text>
        </View>
