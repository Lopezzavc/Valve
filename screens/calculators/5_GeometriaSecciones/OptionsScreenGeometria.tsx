...
// Mapeo de claves de i18n para títulos
const TITLE_I18N_KEY: Record<string, string> = {
  length: 'optionsScreen.titles.length',
  area: 'optionsScreen.titles.area',
  sectionType: 'geometriaSeccionesCalc.labels.sectionType',
};

// Mapeo de claves de i18n para subtítulos
...
const OptionsScreenGeometria = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenGeometria'>>();
  ...
  // Obtener etiqueta mostrada según la categoría y el valor
  const getDisplayLabel = useCallback((cat: string, value: string): string => {
    if (cat === 'sectionType') {
      // Los tipos de sección usan claves de i18n específicas
      return t(`geometriaSeccionesCalc.options.sectionType.${value}`);
    }
    if (cat === 'length' || cat === 'area') {
      // Las unidades se muestran tal cual (son símbolos universales)
      return value;
    }
    return value;
  }, [t]);
...
export default OptionsScreenGeometria;