...
const OptionsScreenDiseño = () => {
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();
  const route = useRoute<RouteProp<RootStackParamList, 'OptionsScreenDiseño'>>();
  const params = route.params ?? { category: 'length' }; // Default a length
  ...
  // Construir título y subtítulo con i18n
  const title = useMemo(() => {
    return t(TITLE_I18N_KEY[category] ?? 'optionsScreen.titles.generic');
  }, [category, t]);

  const subtitle = useMemo(() => {
    return t(SUBTITLE_I18N_KEY[category] ?? 'optionsScreen.subtitles.generic');
  }, [category, t]);

  const [selectedOptionState, setSelectedOption] = useState<string>(
    selectedFromParams && options.includes(selectedFromParams)
      ? selectedFromParams
      : (options[0] ?? 'm') // Default a 'm' si no hay opción
  );
  ...
  const getDisplayLabel = useCallback((cat: string, value: string): string => {
    if (cat === 'length' || cat === 'viscosity' || cat === 'acceleration' || cat === 'flow') {
      return value;
    }
    return value;
  }, [t]);
...
export default OptionsScreenDiseño;